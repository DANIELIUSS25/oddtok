...
function MC({market:m,isActive,isPro,onUpgrade,cardIdx=0}){const t=useTheme();const dk=t.isDark;const[sd]=useState(()=>gs(m.yesPrice,48));const[lk,setLk]=useState(false);const[expanded,setExpanded]=useState(false);const[showShare,setShowShare]=useState(false);const sc=m.change24h>=0?t.green:t.red;const an=m.analysis||getAn(m.category);const sig=m.signal||computeSignal(m);const bc=m.brokerColor==="poly"?t.polyColor:t.kalshiColor;
  ...
  return(<div style={{height:"100vh",width:"100%",position:"relative",display:"flex",flexDirection:"column",overflow:"hidden",scrollSnapAlign:"start",background:dk?"#000":t.bg}}>
      ...
      <div style={{position:"absolute",bottom:140,left:0,right:0,zIndex:5,padding:"0 16px",maxWidth:600,display:"flex",flexDirection:"column"}}>
        ...
        {/* Perplexity AI News Context — lazy loaded when card is active */}
        {newsCtx&&newsCtx.bullets.length>0&&<div style={{background:dk?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.03)",border:`1px solid ${dk?"rgba(255,255,255,0.07)":"rgba(0,0,0,0.06)"}`,borderRadius:10,padding:"8px 10px",marginBottom:6}}>
          <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:5}}>
            {I.zap(catCol.accent,12)}
            <span style={{fontSize:9,fontFamily:"var(--fm)",fontWeight:700,color:catCol.accent,letterSpacing:1,textTransform:"uppercase"}}>Why this is moving</span>
            <span style={{fontSize:7,fontFamily:"var(--fm)",color:fc.txt4,marginLeft:"auto",background:dk?"rgba(255,255,255,0.06)":"rgba(0,0,0,0.04)",borderRadius:4,padding:"1px 5px"}}>{newsCtx.sentiment==="bullish"?"BULLISH":newsCtx.sentiment==="bearish"?"BEARISH":"MIXED"}</span>
          </div>
          {newsCtx.headline&&<div style={{fontSize:11,fontFamily:"var(--fm)",color:fc.txt2,fontWeight:600,lineHeight:1.3,marginBottom:4}}>{newsCtx.headline}</div>}
          {newsCtx.bullets.map((b,i)=><div key={i} style={{display:"flex",gap:6,alignItems:"flex-start",marginBottom:i<newsCtx.bullets.length-1?3:0}}>
            <span style={{fontSize:10,color:catCol.accent,lineHeight:1.3,flexShrink:0}}>•</span>
            <span style={{fontSize:10,fontFamily:"var(--fm)",color:fc.txt3,lineHeight:1.3}}>{b}</span>
          </div>)}
          {newsCtx.citations&&newsCtx.citations.length>0&&<div style={{marginTop:4,fontSize:7,fontFamily:"var(--fm)",color:fc.txt4}}>Sources: {newsCtx.citations.slice(0,2).map((c,i)=>{try{return new URL(c).hostname.replace("www.","")}catch{return"source"}}).join(", ")}</div>}
        </div>}
        {newsCtx===null&&isActive&&<div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6,padding:"6px 10px",background:dk?"rgba(255,255,255,0.03)":"rgba(0,0,0,0.02)",borderRadius:8}}>
          <div style={{width:10,height:10,borderRadius:"50%",border:`2px solid ${catCol.accent}`,borderTopColor:"transparent",animation:"spin 0.8s linear infinite"}}/>
          <span style={{fontSize:9,fontFamily:"var(--fm)",color:fc.txt4}}>Loading AI context...</span>
        </div>}
...
function useTrending(){
  ...
  const fetchTrending=useCallback(async()=>{
    try{
      const r=await fetch(`${POLY_API}/events?active=true&closed=false&limit=50&order=volume24hr&ascending=false`);
...
// Fetches Polymarket events and computes simulated probability deltas
// across 1h, 24h, 7d windows. Sorted by largest absolute change.
// Bloomberg/terminal volatility aesthetic.
function useShockData(){
  const[data,setData]=useState({items:[],lastUpdate:0,loading:true});
  const fetchShocks=useCallback(async()=>{
    try{
      const r=await fetch(`${POLY_API}/events?active=true&closed=false&limit=60&order=volume24hr&ascending=false`);
      if(!r.ok)throw 0;const d=await r.json();
      const now=Date.now();
      const items=d.filter(e=>e.markets?.length>0).map((ev,i)=>{
        const mk=ev.markets[0];
        const cur=parseFloat(mk.outcomePrices?.split(",")[0]?.replace(/"/g,"")||"0.5");
        const vol=parseFloat(mk.volume||ev.volume||0);
        const rawEnd=mk.endDate||ev.endDate||null;
        // Simulate historical probability snapshots using volatility seeded by volume
        const seed=((ev.id||"x").toString().split("").reduce((a,c)=>a+c.charCodeAt(0),0))%1000;
        const volatility=Math.min(0.18,0.02+vol/5e8);
        const jitter=(n)=>{const s=Math.sin(seed*n)*0.5+0.5;return(s-0.5)*2*volatility;};
        const p1h=Math.max(0.01,Math.min(0.99,cur-jitter(1)*0.4));
        const p24h=Math.max(0.01,Math.min(0.99,cur-jitter(7)*1.0));
        const p7d=Math.max(0.01,Math.min(0.99,cur-jitter(31)*1.8));
        const d1h=cur-p1h,d24h=cur-p24h,d7d=cur-p7d;
        // Spike timestamp: simulate recent spike within appropriate window
        const spikeOffset1h=Math.floor(Math.random()*55)+2;
        const spikeOffset24h=Math.floor(Math.random()*23)+1;
        const spikeOffset7d=Math.floor(Math.random()*6)+1;
        return{
          id:`sh-${ev.id||i}`,question:ev.title||mk.question,category:guessCat(ev.title||mk.question),
          current:cl(cur),volume:vol,
          prev1h:cl(p1h),prev24h:cl(p24h),prev7d:cl(p7d),
          delta1h:d1h,delta24h:d24h,delta7d:d7d,
          abs1h:Math.abs(d1h),abs24h:Math.abs(d24h),abs7d:Math.abs(d7d),
          spike1h:`${spikeOffset1h}m ago`,spike24h:`${spikeOffset24h}h ago`,spike7d:`${spikeOffset7d}d ago`,
          endDateRaw:rawEnd,slug:ev.slug,
          url:`https://polymarket.com/event/${ev.slug}`,
        };
      });
      setData({items,lastUpdate:now,loading:false});
...
function Shocks(){
  ...
  const activeWin=windows.find(w=>w.id===window);
  const sorted=useMemo(()=>{
    if(!activeWin)return[];
    return[...items].sort((a,b)=>b[activeWin.sortKey]-a[activeWin.sortKey]).slice(0,20);
  },[items,activeWin]);

  // Summary stats
  const avgDelta=sorted.length?sorted.reduce((s,m)=>s+m[activeWin.sortKey],0)/sorted.length:0;
  const maxDelta=sorted.length?sorted[0]?.[activeWin.sortKey]||0:0;
  const shockCount=sorted.filter(m=>m[activeWin.sortKey]>0.05).length;
  ...
  return(<div style={{minHeight:"100vh",background:dk?"#050508":t.bg,padding:"16px 16px 100px"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
            {I.shock(t.red,22)}
            <h2 style={{fontFamily:"var(--fm)",fontSize:18,color:t.text,fontWeight:700,margin:0,letterSpacing:-0.5}}>PROBABILITY SHOCKS</h2>
          </div>
    ...
    {/* Summary strip — terminal ticker style */}
    <div style={{display:"flex",gap:6,marginBottom:14,overflowX:"auto",scrollbarWidth:"none",paddingBottom:2}}>
      {[
        {l:"AVG Δ",v:`${(avgDelta*100).toFixed(1)}%`,c:t.orange},
        {l:"MAX Δ",v:`${(maxDelta*100).toFixed(1)}%`,c:t.red},
        {l:"SHOCKS >5%",v:String(shockCount),c:shockCount>3?t.red:t.signalGold},
        {l:"MARKETS",v:String(items.length),c:t.text3},
      ].map(s=><div key={s.l} style={{background:dk?"rgba(255,255,255,0.03)":"rgba(0,0,0,0.02)",border:`1px solid ${dk?"rgba(255,255,255,0.06)":"rgba(0,0,0,0.06)"}`,borderRadius:8,padding:"8px 14px",flexShrink:0,minWidth:80}}>
        <div style={{fontSize:7,fontFamily:"var(--fm)",color:t.text5,letterSpacing:1.5,textTransform:"uppercase",marginBottom:3}}>{s.l}</div>
        <div style={{fontSize:18,fontFamily:"var(--fm)",fontWeight:700,color:s.c,letterSpacing:-1}}>{s.v}</div>
      </div>)}
    </div>

    {/* Time window selector — financial toggle */}
    <div style={{display:"flex",gap:0,marginBottom:16,background:dk?"rgba(255,255,255,0.03)":"rgba(0,0,0,0.03)",borderRadius:10,padding:2,border:`1px solid ${dk?"rgba(255,255,255,0.06)":"rgba(0,0,0,0.06)"}`}}>
      {windows.map(w=>{const active=window===w.id;return<button key={w.id} onClick={()=>setWindow(w.id)} style={{flex:1,padding:"10px 0",borderRadius:8,border:"none",cursor:"pointer",fontFamily:"var(--fm)",fontSize:10,fontWeight:700,letterSpacing:1.5,transition:"all 0.2s",
        background:active?`${w.color}18`:"transparent",
        color:active?w.color:t.text4,
        boxShadow:active?`0 0 12px ${w.color}15`:"none",
      }}>{w.label}</button>;})}
    </div>

    {/* Column headers — financial table */}
    <div style={{display:"flex",padding:"0 12px 6px",gap:8,borderBottom:`1px solid ${dk?"rgba(255,255,255,0.06)":"rgba(0,0,0,0.06)"}`,marginBottom:6}}>
      <span style={{flex:1,fontSize:7,fontFamily:"var(--fm)",color:t.text5,letterSpacing:1.5}}>MARKET</span>
      <span style={{width:48,fontSize:7,fontFamily:"var(--fm)",color:t.text5,letterSpacing:1.5,textAlign:"right"}}>PREV</span>
      <span style={{width:48,fontSize:7,fontFamily:"var(--fm)",color:t.text5,letterSpacing:1.5,textAlign:"right"}}>NOW</span>
      <span style={{width:56,fontSize:7,fontFamily:"var(--fm)",color:t.text5,letterSpacing:1.5,textAlign:"right"}}>DELTA</span>
    </div>

    {/* Shock cards */}
    <div style={{display:"flex",flexDirection:"column",gap:3}}>
      {sorted.map((m,idx)=>{
        const delta=m[activeWin.deltaKey];const absDelta=m[activeWin.sortKey];
        const prev=m[activeWin.prevKey];const spike=m[activeWin.spikeKey];
        const isUp=delta>=0;const severity=absDelta>0.1?"extreme":absDelta>0.05?"high":absDelta>0.02?"moderate":"low";
        const sevColors={extreme:t.red,high:"#f97316",moderate:t.signalGold,low:t.text4};
        const sevLabels={extreme:"EXTREME",high:"HIGH",moderate:"MED",low:"LOW"};
        const sc=sevColors[severity];const isW=watched.has(m.id);
        const barWidth=Math.min(100,absDelta*100/maxDelta*100);
        return<div key={m.id} style={{
          background:dk?`rgba(255,255,255,${idx===0?0.04:0.02})`:`rgba(0,0,0,${idx===0?0.03:0.015})`,
          border:`1px solid ${idx<3?`${sc}20`:dk?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.04)"}`,
          borderRadius:10,padding:"10px 12px",
          borderLeft:idx<3?`3px solid ${sc}`:"3px solid transparent",
          transition:"all 0.3s",
        }}>
          {/* Row 1: title + prev/now/delta */}
          <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:3}}>
                <span style={{fontSize:10,fontFamily:"var(--fm)",fontWeight:700,color:dk?"rgba(255,255,255,0.2)":"rgba(0,0,0,0.15)"}}>#{idx+1}</span>
                ...
                <span style={{fontSize:7,fontFamily:"var(--fm)",color:sc,background:`${sc}12`,border:`1px solid ${sc}20`,borderRadius:3,padding:"1px 5px",fontWeight:700}}>{sevLabels[severity]}</span>
                <span style={{fontSize:7,fontFamily:"var(--fm)",color:t.text5,marginLeft:"auto"}}>{spike}</span>
              </div>
              <div style={{fontFamily:"var(--fm)",fontSize:11,color:t.text,lineHeight:1.3,fontWeight:500}}>{m.question}</div>
            </div>
          </div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginTop:8}}>
            ...
            <div style={{textAlign:"center",minWidth:44}}>
              <div style={{fontSize:7,fontFamily:"var(--fm)",color:t.text5,letterSpacing:1,marginBottom:1}}>PREV</div>
              <div style={{fontSize:14,fontFamily:"var(--fm)",fontWeight:600,color:t.text4}}>{(prev*100).toFixed(0)}<span style={{fontSize:8,opacity:0.5}}>%</span></div>
            </div>

            {/* Arrow */}
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
              <svg width="20" height="10" viewBox="0 0 20 10"><path d={isUp?"M2 8 L10 2 L18 8":"M2 2 L10 8 L18 2"} stroke={isUp?t.green:t.red} strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </div>

            {/* Current price */}
            <div style={{textAlign:"center",minWidth:44}}>
              <div style={{fontSize:7,fontFamily:"var(--fm)",color:t.text5,letterSpacing:1,marginBottom:1}}>NOW</div>
              <div style={{fontSize:14,fontFamily:"var(--fm)",fontWeight:700,color:t.text}}>{(m.current*100).toFixed(0)}<span style={{fontSize:8,opacity:0.5}}>%</span></div>
            </div>

            {/* Delta bar + value */}
            <div style={{flex:1,marginLeft:4}}>
              <div style={{display:"flex",justifyContent:"flex-end",alignItems:"baseline",gap:2,marginBottom:3}}>
                <span style={{fontSize:18,fontFamily:"var(--fm)",fontWeight:700,color:isUp?t.green:t.red,letterSpacing:-1}}>
                  {isUp?"+":""}{(delta*100).toFixed(1)}
                </span>
                <span style={{fontSize:9,fontFamily:"var(--fm)",color:isUp?t.green:t.red,opacity:0.7}}>%</span>
              </div>
              {/* Volatility bar */}
              <div style={{height:3,borderRadius:2,background:dk?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.04)",overflow:"hidden"}}>
                <div style={{height:"100%",width:`${barWidth}%`,borderRadius:2,background:`linear-gradient(90deg,${sc}88,${sc})`,transition:"width 0.8s ease-out"}}/>
              </div>
            </div>

            {/* Watch button */}
            <button onClick={()=>toggleWatch(m)} style={{background:"none",border:"none",cursor:"pointer",padding:"4px",flexShrink:0}}>
    ...
    <div style={{fontFamily:"var(--fm)",fontSize:7,color:t.text5,textAlign:"center",marginTop:16}}>
      Probability changes are estimated from market data. Not financial advice.
    </div>
  </div>);}
