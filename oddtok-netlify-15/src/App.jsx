...
//  v12: Narrative Pulse scanner, live ticker, share cards,
...
const I={
  ...
  pulse:(c="#fff",s=22)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 12h4l3-9 4 18 3-9h6"/></svg>,
...
async function fetchPoly(limit=30){try{const r=await fetch(`${POLY_API}?active=true&closed=false&limit=${limit}&order=volume24hr&ascending=false`);if(!r.ok)throw 0;const d=await r.json();return d.filter(e=>e.markets?.length>0).map((ev,i)=>{const m=ev.markets[0];const yp=parseYP(m);
      const rawEnd=m.endDate||ev.endDate||null;return{id:`p-${ev.id||i}`,question:ev.title||m.question,category:guessCat(ev.title||m.question),yesPrice:cl(yp),volume:parseFloat(m.volume||ev.volume||0)||0,liquidity:parseFloat(m.liquidity||0)||0,endDate:rawEnd?new Date(rawEnd).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}):"TBD",endDateRaw:rawEnd,createdIdx:i,image:ev.image||m.image||getMedia(guessCat(ev.title||m.question),i).img,tags:genT(ev.title||m.question),broker:"polymarket",brokerLabel:"Polymarket",brokerColor:"poly",slug:ev.slug,url:`https://polymarket.com/event/${ev.slug}`,participants:Math.floor(Math.random()*40000)+1000,change24h:(Math.random()-0.45)*0.12,comments:Math.floor(Math.random()*15000)+500,shares:Math.floor(Math.random()*5000)+100,trending:parseFloat(m.volume||0)>1e6,sentiment:40+Math.floor(Math.random()*30)}});}catch(e){console.warn("fetchPoly error:",e);return[];}}
async function fetchKalshi(limit=30){try{const r=await fetch(`${KALSHI_API}?status=open&with_nested_markets=true&limit=${limit}`);if(!r.ok)throw 0;const d=await r.json();return(d.events||[]).filter(e=>e.markets?.length>0).map((ev,i)=>{const m=ev.markets[0];let yp=0.5;if(m.yes_bid!=null&&m.yes_ask!=null){yp=((Number(m.yes_bid)+Number(m.yes_ask))/2)/100;}else if(m.last_price!=null){yp=Number(m.last_price)/100;}if(isNaN(yp)||!isFinite(yp))yp=0.5;const rawEnd=m.close_time||null;return{id:`k-${ev.event_ticker||i}`,question:ev.title||m.title,category:mapKCat(ev.category),yesPrice:cl(yp),volume:Number(m.volume)||0,liquidity:Number(m.open_interest)||0,endDate:rawEnd?new Date(rawEnd).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}):"TBD",endDateRaw:rawEnd,createdIdx:i+30,image:getMedia(mapKCat(ev.category),i).img,tags:genT(ev.title||m.title),broker:"kalshi",brokerLabel:"Kalshi",brokerColor:"kalshi",slug:ev.event_ticker,url:`https://kalshi.com/markets/${ev.event_ticker}`,participants:Math.floor(Math.random()*20000)+500,change24h:(Math.random()-0.45)*0.1,comments:Math.floor(Math.random()*8000)+200,shares:Math.floor(Math.random()*3000)+50,trending:(Number(m.volume)||0)>5000,sentiment:35+Math.floor(Math.random()*35)}});}catch(e){console.warn("fetchKalshi error:",e);return[];}}
...
function cl(v){const n=Number(v);return(isNaN(n)||!isFinite(n))?0.5:Math.max(0.01,Math.min(0.99,n));}
// Robust parser for Polymarket outcomePrices (handles string, JSON, array, number)
function parseYP(m){try{const op=m.outcomePrices;if(!op)return 0.5;if(typeof op==="number")return op;if(Array.isArray(op))return parseFloat(op[0])||0.5;if(typeof op==="string"){const c=op.replace(/[\[\]"]/g,"");return parseFloat(c.split(",")[0])||0.5;}return 0.5;}catch{return 0.5;}}
...
function gs(b,l=48){const base=(isNaN(b)||!isFinite(b))?0.5:b;const d=[base];for(let i=1;i<l;i++)d.push(Math.max(0.01,Math.min(0.99,d[i-1]+(Math.random()-0.5)*0.03)));return d;}
...
function Spark({data,color,width=160,height=50}){const safe=data.map(v=>(isNaN(v)||!isFinite(v))?0.5:v);const min=Math.min(...safe),max=Math.max(...safe),rng=max-min||1;const pts=safe.map((v,i)=>`${(i/(safe.length-1))*width},${height-((v-min)/rng)*height}`).join(" ");const uid=`s${Math.random().toString(36).slice(2,8)}`;return<svg width={width} height={height} style={{overflow:"visible"}}><defs><linearGradient id={uid} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={color} stopOpacity="0.25"/><stop offset="100%" stopColor={color} stopOpacity="0"/></linearGradient></defs><polygon points={`0,${height} ${pts} ${width},${height}`} fill={`url(#${uid})`}/><polyline points={pts} fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>;}
  ...
  const fetchTrending=useCallback(async()=>{
    try{
      ...
      const all=d.filter(e=>e.markets?.length>0).map((ev,i)=>{
        const mk=ev.markets[0];const yp=parseYP(mk);
...
// ═══ PULSE — NARRATIVE VELOCITY SCANNER ═════════════
// Scans X/Twitter via Perplexity for fastest-spreading discussions.
// Categories: AI, Crypto, Elections, Tech, Geopolitics.
// Ranked by velocity of mentions over last 6 hours.
const PULSE_CATS=[
...
const VELOCITY_COLORS=["#ef4444","#f97316","#eab308","#22c55e","#06b6d4","#8b5cf6","#ec4899","#f43f5e","#14b8a6","#a855f7"];
const SENT_CONFIG={bullish:{color:"#00ff88",icon:"▲",label:"BULLISH"},bearish:{color:"#ff3b30",icon:"▼",label:"BEARISH"},neutral:{color:"#888",icon:"◆",label:"NEUTRAL"},divided:{color:"#f97316",icon:"⚡",label:"DIVIDED"}};
...
function Pulse(){
  ...
  const fetchNarratives=useCallback(async(c)=>{
    ...
    try{
      ...
      if(d.narratives&&d.narratives.length>0){
        ...
        setLastScan(d.scanTime||new Date().toISOString());
      }else{
        setError(d.error||"No narratives found");
      }
    }catch(e){setError(e.message);}
    finally{setLoading(false);}
  },[]);

  useEffect(()=>{fetchNarratives(cat);},[cat,fetchNarratives]);

  // Auto-refresh every 5 minutes
  useEffect(()=>{const iv=setInterval(()=>fetchNarratives(cat),300000);return()=>clearInterval(iv);},[cat,fetchNarratives]);

  const maxVel=narratives.length?Math.max(...narratives.map(n=>n.velocity||0),1):100;

  return(<div style={{minHeight:"100vh",background:dk?"#030306":t.bg,padding:"16px 16px 100px"}}>
    {/* Header */}
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
      <div>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
          {I.pulse(t.accent,22)}
          <h2 style={{fontFamily:"var(--fm)",fontSize:18,color:t.text,fontWeight:700,margin:0,letterSpacing:-0.5}}>NARRATIVE PULSE</h2>
        </div>
        <p style={{fontFamily:"var(--fm)",fontSize:9,color:t.text4,margin:0,letterSpacing:1}}>X / TWITTER SCANNER &middot; LAST 6 HOURS</p>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:4}}>
        {loading?<div style={{width:8,height:8,borderRadius:"50%",border:`2px solid ${t.accent}`,borderTopColor:"transparent",animation:"spin 0.8s linear infinite"}}/>:I.live(6)}
        <span style={{fontSize:8,fontFamily:"var(--fm)",color:loading?t.text4:t.green,letterSpacing:1}}>{loading?"SCANNING":"LIVE"}</span>
      </div>
    </div>

    {/* Category filter */}
    <div style={{display:"flex",gap:4,marginBottom:16,overflowX:"auto",scrollbarWidth:"none",paddingBottom:2}}>
      {PULSE_CATS.map(c=>{const active=cat===c.id;return<button key={c.id} onClick={()=>setCat(c.id)} style={{display:"flex",alignItems:"center",gap:4,padding:"6px 12px",borderRadius:8,cursor:"pointer",border:active?`1px solid ${t.accent}30`:`1px solid ${dk?"rgba(255,255,255,0.06)":"rgba(0,0,0,0.06)"}`,background:active?`${t.accent}12`:dk?"rgba(255,255,255,0.03)":"rgba(0,0,0,0.02)",fontFamily:"var(--fm)",fontSize:10,fontWeight:active?700:400,color:active?t.accent:t.text3,transition:"all 0.2s",flexShrink:0,whiteSpace:"nowrap"}}>
        <span style={{fontSize:12}}>{c.icon}</span>{c.label}
      </button>;})}
    </div>

    {/* Loading state */}
    {loading&&narratives.length===0&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"60px 20px"}}>
      <div style={{position:"relative",width:60,height:60,marginBottom:16}}>
        ...
        <div style={{position:"absolute",inset:0,borderRadius:"50%",border:`2px solid transparent`,borderTopColor:t.accent,animation:"spin 1s linear infinite"}}/>
        {I.pulse(t.accent,24)}
      </div>
      <div style={{fontFamily:"var(--fm)",fontSize:12,color:t.text3,letterSpacing:2}}>SCANNING X</div>
      <div style={{fontFamily:"var(--fm)",fontSize:9,color:t.text4,marginTop:4}}>Analyzing narrative velocity...</div>
    </div>}

    {/* Error state */}
    {error&&!loading&&narratives.length===0&&<div style={{textAlign:"center",padding:"40px 20px"}}>
      <div style={{fontFamily:"var(--fm)",fontSize:12,color:t.red,marginBottom:8}}>Scan failed</div>
      <div style={{fontFamily:"var(--fm)",fontSize:10,color:t.text4,marginBottom:12}}>{error==="500"?"Set PERPLEXITY_API_KEY in Netlify env":error}</div>
      <button onClick={()=>fetchNarratives(cat)} style={{padding:"8px 16px",borderRadius:8,border:`1px solid ${t.accent}30`,background:`${t.accent}12`,fontFamily:"var(--fm)",fontSize:10,color:t.accent,fontWeight:700,cursor:"pointer"}}>Retry Scan</button>
    </div>}

    {/* Narrative cards */}
    {narratives.length>0&&<div style={{display:"flex",flexDirection:"column",gap:6}}>
      {narratives.map((n,idx)=>{
        const vel=n.velocity||0;
        const velPct=Math.round(vel/maxVel*100);
        const barColor=VELOCITY_COLORS[idx%VELOCITY_COLORS.length];
        const sent=SENT_CONFIG[n.sentiment]||SENT_CONFIG.neutral;
        const catIcon=PULSE_CATS.find(c=>c.id===n.category)?.icon||"📊";

        return<div key={idx} style={{background:dk?`rgba(255,255,255,${idx===0?0.05:0.025})`:`rgba(0,0,0,${idx===0?0.03:0.015})`,border:`1px solid ${idx<3?`${barColor}25`:dk?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.04)"}`,borderLeft:idx<3?`3px solid ${barColor}`:"3px solid transparent",borderRadius:10,padding:"12px 14px",transition:"all 0.3s"}}>

          {/* Rank + Category + Sentiment + Velocity score */}
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
            <span style={{fontSize:20,fontFamily:"var(--fm)",fontWeight:700,color:barColor,lineHeight:1,minWidth:28}}>#{n.rank||idx+1}</span>
            <span style={{fontSize:10}}>{catIcon}</span>
            <span style={{fontSize:7,fontFamily:"var(--fm)",color:t.text5,background:dk?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.03)",borderRadius:3,padding:"1px 5px",textTransform:"uppercase",letterSpacing:1}}>{n.category}</span>
            <span style={{fontSize:7,fontFamily:"var(--fm)",color:sent.color,background:`${sent.color}12`,border:`1px solid ${sent.color}20`,borderRadius:3,padding:"1px 5px",fontWeight:700}}>{sent.icon} {sent.label}</span>
            <span style={{marginLeft:"auto",fontSize:22,fontFamily:"var(--fm)",fontWeight:700,color:barColor,letterSpacing:-2,lineHeight:1}}>{vel}</span>
          </div>

          {/* Title */}
          <div style={{fontFamily:"var(--fm)",fontSize:13,color:t.text,fontWeight:600,lineHeight:1.3,marginBottom:4}}>{n.title}</div>

          {/* Description */}
          <div style={{fontFamily:"var(--fm)",fontSize:10,color:t.text3,lineHeight:1.4,marginBottom:8}}>{n.description}</div>

          {/* Velocity bar */}
          <div style={{marginBottom:8}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
              <span style={{fontSize:7,fontFamily:"var(--fm)",color:t.text5,letterSpacing:1.5}}>VELOCITY</span>
              <span style={{fontSize:8,fontFamily:"var(--fm)",color:barColor,fontWeight:700}}>{vel}/100</span>
            </div>
            <div style={{height:4,borderRadius:2,background:dk?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.04)",overflow:"hidden"}}>
              <div style={{height:"100%",width:`${velPct}%`,borderRadius:2,background:`linear-gradient(90deg,${barColor}88,${barColor})`,transition:"width 1s ease-out"}}/>
            </div>
          </div>

          {/* Key phrases as pills */}
          {n.keyPhrases&&n.keyPhrases.length>0&&<div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>
            {n.keyPhrases.map((kp,i)=><span key={i} style={{fontSize:8,fontFamily:"var(--fm)",color:t.accent,background:`${t.accent}10`,border:`1px solid ${t.accent}20`,borderRadius:6,padding:"2px 8px",fontWeight:600}}>#{kp.replace(/^#/,"")}</span>)}
          </div>}

          {/* Sample tweet */}
          {n.sampleTweet&&<div style={{background:dk?"rgba(255,255,255,0.03)":"rgba(0,0,0,0.02)",border:`1px solid ${dk?"rgba(255,255,255,0.05)":"rgba(0,0,0,0.04)"}`,borderRadius:8,padding:"8px 10px"}}>
            <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:4}}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill={t.text4}><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
              <span style={{fontSize:8,fontFamily:"var(--fm)",color:t.text5,letterSpacing:1}}>SAMPLE POST</span>
            </div>
            <div style={{fontSize:10,fontFamily:"var(--fm)",color:t.text3,lineHeight:1.4,fontStyle:"italic"}}>"{n.sampleTweet}"</div>
          </div>}
        </div>;
      })}
    </div>}
    ...
    {/* Scan timestamp */}
    {lastScan&&<div style={{fontFamily:"var(--fm)",fontSize:8,color:t.text5,textAlign:"center",marginTop:14}}>
      Last scan: {new Date(lastScan).toLocaleTimeString()} &middot; Refreshes every 5 min &middot; Powered by Perplexity AI
    </div>}
  </div>);}
...
export default function OddTokApp(){
  ...
  const nav=[{id:"feed",icon:I.home,l:"Feed"},{id:"pulse",icon:I.pulse,l:"Pulse"},{id:"trending",icon:I.trending,l:"Trending"},{id:"watchlist",icon:I.bell,l:"Watchlist"},{id:"profile",icon:I.user,l:"Profile"}];
  return(
    ...
    <div style={{width:"100%",minHeight:"100vh",background:t.bg,position:"relative",transition:"background 0.3s"}}>
      ...
      <div>{pg==="feed"&&<Feed markets={mkts} loading={ld} isPro={isPro} onUpgrade={onUpgrade}/>}{pg==="pulse"&&<Pulse/>}{pg==="trending"&&<Trending/>}{pg==="shocks"&&<Shocks/>}{pg==="watchlist"&&<Watchlist markets={mkts}/>}{pg==="markets"&&<Markets markets={mkts}/>}{pg==="leaderboard"&&<Ranks/>}{pg==="profile"&&<Profile isDark={isDark} toggleTheme={toggleTheme} isPro={isPro} onUpgrade={onUpgrade} user={user} onAuth={onAuth}/>}</div>
